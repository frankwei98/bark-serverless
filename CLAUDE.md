...
- **`worker/src/index.ts`** is the *only* place that wires real implementations: it builds `KVDeviceRegistry`, routes each device key through `DeviceRegistryCoordinator`, builds `CloudflareApnsClient` and the real clock, then caches the assembled Hono app in a `WeakMap` keyed on `env`. The per-key Durable Object owns the authoritative registration and serializes conditional invalid-token cleanup. State changes atomically record a pending KV mirror and its alarm; failed mirrors retry with backoff without rolling back the registration. Keep external KV I/O outside storage transactions and update in-memory caches only after transaction commit. The app cache means the APNs client (and its imported crypto key + cached JWT) persists across requests within a Worker isolate — don't move client construction into the request path.
...
Config is env-driven (`BarkBindings` in `types.ts`, assembled by `config.ts`). `wrangler.toml` holds `[vars]` (APNS_*, optional `URL_PREFIX`, `BASIC_AUTH_*`, `MAX_BATCH_PUSH_COUNT`, `MAX_REQUEST_BODY_BYTES`), the `DEVICE_REGISTRY` KV binding, and the `DEVICE_REGISTRY_COORDINATOR` Durable Object binding plus migration. KV mirrors devices as `device:<key> -> <token>` for enumeration and legacy lookups. Existing DO registration records remain readable, and KV-only devices are adopted when registered. `/info` counts the KV mirror and may lag acknowledged registrations while synchronization is pending.
